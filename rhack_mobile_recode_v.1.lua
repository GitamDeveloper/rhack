...
local Window = Library.CreateLib("RHACK mobile recode v.1", "Midnight")
...
Section:NewToggle("Fly", "Fly on/off", function(state_foo)
    if state_foo then
...
Section:NewToggle("no clip", "nc on/off", function(state_nc)
    if state_nc then
        nc_oo = true
        eplayer.Character.HumanoidRootPart.Anchored = true
...
Section:NewButton("no clip", "no clip", function()
    if nc_d == "up" then
        if nc_oo == true then
...
Section:NewDropdown("no clip", "no clip direction", {"up", "down","right","left","forward","back",}, function(currentOption)
    if currentOption == "up" then
        nc_d = "up"
    end

    if currentOption == "down" then
        nc_d = "down"
    end

    if currentOption == "right" then
        nc_d = "right"
    end

    if currentOption == "left" then
        nc_d = "left"
    end

    if currentOption == "forward" then
        nc_d = "forward"
    end

    if currentOption == "back" then
        nc_d = "back"
    end
end)

local Section = Tab:NewSection("ACC")


Section:NewToggle("AAC", "Anti Anti Cheat", function(state_aac)
    if state_aac then
        aac_oo = true
    else
        aac_oo = false
